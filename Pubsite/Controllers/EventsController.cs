...
namespace Pubsite.Controllers
{
    ...
    public class EventsController : Controller
    {
        ...
        public ActionResult Index(int? page, string searchText = "", string Type="")
        {
            ...
            {
              events = db.Events.Include(e => e.ContentDetail).ToList().Where(e => (e.ContentDetail.Title.Contains(searchText.ToString())) && (e.EventType.Contains(Type))).OrderByDescending(e=>e.EventID).ToList().ToPagedList(page ?? 1, 10);
            }
            else
            {
              events = db.Events.Include(e => e.ContentDetail).ToList().Where(e => e.ContentDetail.Title.Contains(searchText.ToString())).OrderByDescending(e=>e.EventID).ToList().ToPagedList(page ?? 1, 10);
            }
            return View(events);
        }
