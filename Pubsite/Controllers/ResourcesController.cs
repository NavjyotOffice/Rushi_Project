...
namespace Pubsite.Controllers
{
    ...
    public class ResourcesController : Controller
    {
        ...
        // GET: Resources
        public ActionResult Index(int? page, string searchText="", string Type = "")
        {
            IPagedList<Resource> resources;
            searchText = searchText.Trim();
            Type = Type.Trim();
            if (!string.IsNullOrEmpty(Type))
            {
              resources = db.Resources.Include(r => r.ContentDetail).ToList().Where(r => (r.ContentDetail.Title.Contains(searchText.ToString())) && (r.ResourceType.Contains(Type))).OrderByDescending(r=>r.ResourcesID).ToList().ToPagedList(page ?? 1, 10);
            }
            else
            {
              resources = db.Resources.Include(r => r.ContentDetail).ToList().Where(r => r.ContentDetail.Title.Contains(searchText.ToString())).OrderByDescending(r=>r.ResourcesID).ToList().ToPagedList(page ?? 1, 10);
            }
        ...
        public ActionResult Create(Resource resource)
        {
            if (ModelState.IsValid)
            {
                if (resource.ContentDetail.Upload != null && resource.ContentDetail.Upload.ContentLength > 0)
                {
                    ...
                    string path = Path.Combine(Server.MapPath("~/Uploads/Resources/Images"), fileName);
                    resource.ContentDetail.Upload.SaveAs(path);
                    resource.ContentDetail.Image = "Uploads/Resources/Images/" + fileName;
                }
                ...
                if (resource.Upload != null && resource.Upload.ContentLength > 0)
                {
                    ...
                    string path = Path.Combine(Server.MapPath("~/Uploads/Resources/Resource"), fileName);
                    resource.Upload.SaveAs(path);
                    resource.ResourceFile = "Uploads/Resources/Resource/" + fileName;
                }
