...
namespace Pubsite.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        {
            IPagedList<News> news;
            searchText = searchText.Trim();
            Type = Type.Trim();
            if (!string.IsNullOrEmpty(Type))
            {
              news = db.News.Include(n => n.ContentDetail).ToList().Where(n => (n.ContentDetail.Title.Contains(searchText.ToString())) && (n.NewsType.Contains(Type))).OrderByDescending(n=>n.NewsID).ToList().ToPagedList(page ?? 1, 10);
            }
            else
            {
                news = db.News.Include(n => n.ContentDetail).ToList().Where(n => n.ContentDetail.Title.Contains(searchText.ToString())).OrderByDescending(n=>n.NewsID).ToList().ToPagedList(page ?? 1, 10);
            }
            return View(news);
        }
